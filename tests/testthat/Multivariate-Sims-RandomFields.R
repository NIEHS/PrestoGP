...
d12 <- fields::rdist(expand.grid(space.dim/ranges[1],space.dim/ranges[1], time.dim/ranges[2]),
                     expand.grid(space.dim/ranges[3],space.dim/ranges[3], time.dim/ranges[4]))
d21 <- t(d12)

d13 <- fields::rdist(expand.grid(space.dim/ranges[1],space.dim/ranges[1], time.dim/ranges[2]),
                     expand.grid(space.dim/ranges[5],space.dim/ranges[5], time.dim/ranges[6]))
d31 <- t(d13)

d23 <- fields::rdist(expand.grid(space.dim/ranges[3],space.dim/ranges[3], time.dim/ranges[4]),
                     expand.grid(space.dim/ranges[5],space.dim/ranges[5], time.dim/ranges[6]))
d32 <- t(d23)


Dists.All <- rbind( cbind(d11,d12,d13),
                    cbind(d21,d22,d23),
                    cbind(d31,d32,d33)
)
...
set.seed(10)
nug.error <- cbind( nuggets[1]*rnorm(n.spatial*n.temporal),
...
y.sim.final <- cbind(mean.trend.st[idx.outcome1] + st.error[idx.outcome1] + nug.error[idx.outcome1],
...
df.plot <- data.frame("x.coord" = xyt.grid[,1],
                      "y.coord" = xyt.grid[,2],
...
ggplot(df.plot,aes(x.coord,y.coord,fill = mean.trend1))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

ggplot(df.plot,aes(x.coord,y.coord,fill = mean.trend2))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

ggplot(df.plot,aes(x.coord,y.coord,fill = mean.trend3))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

# plot the st error
ggplot(df.plot,aes(x.coord,y.coord,fill = sterror1))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

ggplot(df.plot,aes(x.coord,y.coord,fill = sterror2))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")
...
ggplot(df.plot,aes(x.coord,y.coord,fill = sterror3))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

# plot the nugget
ggplot(df.plot,aes(x.coord,y.coord,fill = nugerror1))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

ggplot(df.plot,aes(x.coord,y.coord,fill = nugerror2))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")

ggplot(df.plot,aes(x.coord,y.coord,fill = nugerror3))+geom_raster(interpolate = T)+
  facet_wrap(.~time)+scale_fill_viridis_c(direction = -1, option = "A")
