test_that("beta.hat not numeric", {
  ...
  expect_error(prestogp_fit(pgp.model1, y, X, locs, beta.hat="foo"),
...
test_that("beta.hat not a vector", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, beta.hat=matrix(1:3)),
               "beta.hat parameter must be a numeric vector")
})
...
test_that("beta.hat incorrect length", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, beta.hat=1:3),
...
test_that("tol not numeric", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, tol="foo"),
               "tol must be numeric")
})
...
test_that("tol not a scalar", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, tol=1:2),
...
test_that("tol out of range", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, tol=1.1),
...
test_that("length(scaling) != ncol(locs)", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, scaling=1:5),
...
test_that("scaling not sequential", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, scaling=c(1,3)),
...
test_that("Invalid Apanasovich model", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, scaling=1:2,
                            apanasovich=TRUE),
...
test_that("covparams not numeric", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, covparams="foo"),
...
test_that("covparams not a vector", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, covparams=matrix(1:3)),
               "covparams must be a numeric vector")
})
...
test_that("covparams incorrect length", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs, covparams=1:3),
...
test_that("covparams incorrect length - multivariate", {
  ...
  expect_error(prestogp_fit(pgp.mmodel1, y.list, X.st, locs.list,
                            covparams=c(0.9, 0.9, 0.5, 0.5, 0.5, 0.5,
                                        0.1, 0.1)),
               "Incorrect number of parameters in covparams")
})
...
test_that("Invalid initial variance estimate", {
  source("sim_vecchia_small.R")
  pgp.model1 <- new("VecchiaModel")
  expect_error(prestogp_fit(pgp.model1, y, X, locs,
...
test_that("m too large", {
  ...
  expect_warning(prestogp_fit(pgp.model1, y, X, locs),
