...
test_that("Invalid Y input", {
  model <- new("VecchiaModel")
  expect_error(prestogp_fit(model, "y", as.matrix(1:3),
                            as.matrix(1:3)),
...
test_that("Invalid X input", {
  model <- new("VecchiaModel")
  expect_error(prestogp_fit(model, as.matrix(1:3), 1:3,
                            as.matrix(1:3)),
               "X must be a matrix")
})
...
test_that("Y has multiple columns", {
  model <- new("VecchiaModel")
  expect_error(prestogp_fit(model, matrix(1:4, nrow=2), as.matrix(1:4),
                            as.matrix(1:4)),
...
test_that("nrow(Y) != nrow(locs)", {
  model <- new("VecchiaModel")
  expect_error(prestogp_fit(model, as.matrix(1:4), as.matrix(1:4),
                            as.matrix(1:3)),
...
test_that("nrow(Y) != nrow(X)", {
  model <- new("VecchiaModel")
  expect_error(prestogp_fit(model, as.matrix(1:4), as.matrix(1:3),
                            as.matrix(1:4)),
...
test_that("Simulated dataset spatiotemporal", {
    ...
    pgp.model1 <- new("VecchiaModel", n_neighbors=25)
    pgp.model1 <- prestogp_fit(pgp.model1, y, X, locs, scaling=c(1,1,2),
    ...
    pgp.model2 <- prestogp_fit(pgp.model2, y, X, locs, scaling=c(1,1,2),
                               apanasovich=FALSE, verbose=FALSE,
                               optim.control=list(trace=0,maxit=5000,
                                                  reltol=1e-3))
    beta.out2 <- as.vector(pgp.model2@beta)
...
test_that("Invalid locs input for prediction", {
    ...
    pgp.model1 <- prestogp_fit(pgp.model1, y.otr, X.otr,
                               locs.otr, scaling=c(1,1),
                               apanasovich=TRUE, verbose=FALSE,
                               optim.control=list(trace=0,maxit=5000,
                                                  reltol=1e-3))
    expect_error(prestogp_predict(pgp.model1, X.otst, 1:3),
                 "locs must be a matrix")
})
...
test_that("Invalid X input for prediction", {
    ...
    pgp.model1 <- prestogp_fit(pgp.model1, y.otr, X.otr,
                               ...
                               optim.control=list(trace=0,maxit=5000,
                                                  reltol=1e-3))
    expect_error(prestogp_predict(pgp.model1, 1:3, locs.otst),
                 "X must be a matrix")
})
...
test_that("ncol(locs) != ncol(locs_train)", {
    ...
    expect_error(prestogp_predict(pgp.model1, X.otst, locs.otst),
...
test_that("m too small for prediction", {
    ...
    pgp.model1 <- prestogp_fit(pgp.model1, y.otr, X.otr,
                               ...
                               optim.control=list(trace=0,maxit=5000,
                                                  reltol=1e-3))
    expect_error(prestogp_predict(pgp.model1, X.otst, locs.otst, m=2),
...
test_that("m too large for prediction", {
    ...
    pgp.model1 <- prestogp_fit(pgp.model1, y.otr, X.otr,
                               ...
                               optim.control=list(trace=0,maxit=5000,
                                                  reltol=1e-3))
    expect_warning(prestogp_predict(pgp.model1, X.otst, locs.otst, m=201),
