...
setMethod("show", "PrestoGPModel", # TODO consider exporting this
          function(object) {
            ...
            if(is.null(Y_names)){
              Y_names <- unlist(lapply(1:ncol(object@Y_train), function(x){paste("Outcome", x)}))
            }
            ...
            if(is.null(X_names)){
              X_names <- unlist(lapply(1:ncol(object@X_train), function(x){paste("Ind. Variable", x)}))
            }
