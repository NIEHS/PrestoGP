...
negloglik_vecchia_ST <- function(logparms, res, vecchia.approx, param.seq, scaling, nscale) {
  parms <- unlog_params(logparms, param.seq, 1)
...
negloglik.full <- function(logparams, d, y, param.seq) {
  params <- unlog_params(logparams, param.seq, 1)
  N <- nrow(d)
...
vecchia_likelihood <- function(z, vecchia.approx, covparms, nuggets,
  ...
  res <- try(junk <- vecchia_likelihood_U_cpp(z, U.obj), silent = TRUE)
...
#unlog_params <- function(logparams, param.seq, P) {
#  params <- c(
